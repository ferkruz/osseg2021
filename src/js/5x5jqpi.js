...
(function(){
    ...
    $.fn.initUploader = function(o) {
        ...
        $ulObj.on('click change', 'input[value]', (e) => {
            if (e.currentTarget.attributes['value'].nodeValue == 'Eliminar') {
    ...
    function queueFiles(fl){
        if (manifest.length == opts.fileLimit || opts.fileLimit < manifest.length + fl.length) {
            const limitMsg = `The limit for number of file uploads isLimite ${opts.fileLimit}.`;
            const $modal = $('.modal');
        ...
        for (let i =0; i < fl.length; i++) {
            if (fl[i].size > (opts.sizeLimit * 1048576)) {
                const sizeMsg = `
                    El tamaño limite por archivo es de ${opts.sizeLimit} MB.
